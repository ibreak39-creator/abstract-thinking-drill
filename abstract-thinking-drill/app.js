...
const storageKey = "abstract-thinking-drill-general-state";
const roundSize = 5;
...
function checkAnswer() {
  ...
  if (attempted) {
    ...
    state.correct[getQuestionId()] =
      state.level === "beginner" ? isSameAnswer(state.responses[getQuestionId()], question.correct) : solved;
    saveState();
  }
...
function renderFinalResult() {
  ...
  if (state.level === "beginner") {
    elements.finalTitle.textContent = "初級の結果";
    elements.finalScore.textContent = `${correct}/${total}`;
    elements.finalMessage.textContent = getBeginnerResultMessage(correct, total);
    renderBeginnerBreakdown();
  } else {
    const grades = getAdvancedGrades();
    const score = grades.reduce((sum, grade) => sum + grade.score, 0);
    const maxScore = grades.length * 3;
    elements.finalTitle.textContent = "上級の結果";
...
function renderBeginnerBreakdown() {
  elements.scoreBreakdown.hidden = false;
  elements.scoreBreakdown.innerHTML = "";
...
function getBeginnerResultMessage(correct, total) {
  if (correct === total) {
